...
2. **职责分离**：中书省只负责开发步骤与架构规划，门下省负责审核，尚书省负责把规划映射成具体部门派发和调度。
3. **原生 Tool Calling**：礼部/兵部/工部可直接读写 `work/`，兵部与工部均可执行命令。
...
- 默认**先确定项目名**，再在 `work/<项目名>/` 下开展工作。
- 若用户明确指定项目名，直接使用该名称；若未指定，应根据需求生成一个简洁、稳定、可读的项目目录名。
- 礼部、兵部、工部的真实产物都应写入对应项目目录，例如 `work/flask-api/README.md`、`work/flask-api/app.py`、`work/flask-api/frontend/src/App.jsx`。
- `work/` 根目录默认只作为项目工作区的容器使用，不应直接把代码、文档、配置散落在根目录。
- 仅在用户明确要求“直接写到 work 根目录”或任务本身就是修改已有根目录文件时，才允许跳过项目子目录。
...
- **兵部工部职责调整**：将兵部与工部改为**通用编码与指令执行**角色，不再严格区分前端/后端
  - 尚书省可根据任务复杂度决定是否并发派发：
    - **简单任务**：单个部门（兵部或工部）独立完成
    - **复杂任务**：并发派发至兵部 + 工部，尚书省为其划分开发模块，实现协作开发
  - 两部门均可使用 `write_file`、`read_file`、`execute_command` 工具
- **户部转型为数据收集角色**：承接网页爬取、API 调用、公开数据搜集，为兵部/工部提供数据支撑
- **吏部监察机制**：吏部将用于在工作过程中监察各部门工作是否跑偏，是否失职
- **分步多轮开发**：每轮由上级验收一次
- **Web 可视化监控**
